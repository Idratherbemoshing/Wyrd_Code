...
def modify_delay_in_file(file_path, new_delay, verbose, make_backup):
    ...
    try:
        ...
        # regular expression to match 'QUACK DELAY' or 'Q DELAY'
        modified_content = re.sub(r'(QUACK DELAY|Q DELAY) \d+', f'\\1 {new_delay}', content, flags=re.IGNORECASE)
...
def modify_all_ducky_scripts(directory, new_delay, file_extension='.txt', verbose=False, make_backup=False):
    """ Modifies all Bash Bunny scripts in the given directory. """
    for root, dirs, files in os.walk(directory):
        for filename in files:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Modify DELAY commands in Bash Bunny scripts.')
    parser.add_argument('-d', '--directory', required=True, help='Directory containing Bash Bunny scripts - this will modify all sub folders so be careful')
